...
int[,] numbersResul = new int[rowsFirstMatrix, columnsSecondMatrix];

for (int j = 0; j < numbersResul.GetLength(1); j++)
{
int sum = 0;
int multipl = 1;
for (int i = 0; i < numbersResul.GetLength(0); i++)
{
for (int k = 0; k < firstMatrix.GetLength(1); k++)
{
multipl = 1;
...
numbersResul[i, j] = sum;
sum = 0;
}
}
WriteMatrix(numbersResul);
